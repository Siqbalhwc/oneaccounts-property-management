# ============================================================================
# apply_fix.ps1 — applies the "Tower suspension + feature flags" code changes
#
# HOW TO USE:
#   1. Put this file AND "tower_suspension_feature_flags.patch" into the
...
#      to redeploy your backend and frontend automatically.
#
...
if (-not (Test-Path "tower_suspension_feature_flags.patch")) {
    Fail "Can't find 'tower_suspension_feature_flags.patch' in this folder. Make sure you saved it here, next to this script."
}
...
git apply --check tower_suspension_feature_flags.patch
if ($LASTEXITCODE -ne 0) {
...
git apply tower_suspension_feature_flags.patch
if ($LASTEXITCODE -ne 0) {
...
git add -A
git commit -m "Add company/user suspension and feature flags (Tower)"
if ($LASTEXITCODE -ne 0) {
...
Write-Host "IMPORTANT: this only did the CODE half. You still need to run the SQL file" -ForegroundColor Yellow
Write-Host "(001_run_this_in_supabase.sql) in Supabase's SQL Editor -- see the separate instructions." -ForegroundColor Yellow
Write-Host "============================================================" -ForegroundColor Green
