# ============================================================================
# apply_tenant_search_fix.ps1 — adds a search bar to the Tenants list page
#
# HOW TO USE:
#   1. Put this file AND "tenant_search.patch" into the main folder of your
...
#   3. Run:
#      powershell -ExecutionPolicy Bypass -File apply_tenant_search_fix.ps1
#
# NOTE: this is a frontend-only change (no database change needed) --
# search happens instantly in the browser over tenants already loaded,
# filtering by name, CNIC, or address.
#
...
if (-not (Test-Path "tenant_search.patch")) {
    Fail "Can't find 'tenant_search.patch' in this folder. Make sure you saved it here, next to this script."
}
...
git apply --check tenant_search.patch
if ($LASTEXITCODE -ne 0) {
...
git add -A
git commit -m "Add search bar to tenant list (name, CNIC, address)"
if ($LASTEXITCODE -ne 0) {
