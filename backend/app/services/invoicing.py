...
def resync_current_month_invoice(supabase: Client, company_id: str, lease: dict) -> Optional[dict]:
    ...
    # "Currently relevant to this invoice's month" -- either still open
    # (effective_to null) or was open at some point during this month.
    relevant = [
        c for c in charges
        if date.fromisoformat(str(c["effective_from"])) <= (
            date(month_start.year + 1, 1, 1) - timedelta(days=1) if month_start.month == 12
            else date(month_start.year, month_start.month + 1, 1) - timedelta(days=1)
        )
        and (c.get("effective_to") is None or date.fromisoformat(str(c["effective_to"])) >= month_start)
    ]
    ...
    prorated = compute_prorated_charges(relevant, lease_start, lease_end, month_start, is_first_invoice)
