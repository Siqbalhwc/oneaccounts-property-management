...
def _self_heal_allocations_for_range(supabase: Client, date_from: date, date_to: date) -> None:
    """
    Allocates any invoice-tied payment dated within [date_from, date_to]
    that doesn't have a payment_allocations row yet, using the exact same
    allocate_payment_to_line_items() function live receipts already call --
    so a payment healed here and one allocated the moment it was received
    are always split identically. Processed oldest-first so a lease with
    several partial payments in the window still sees the correct
    already-allocated balance left by the payment before it.

    Best-effort and silent by design: called on every GET of the report
    (cheap once everything in range is already allocated -- it's then just
    one query that finds nothing to do), and a failure here must never
    break the report itself, only leave it relying on the reconciliation
    banner / manual backfill for that one gap.
    """
    try:
        payments_in_range = (
            supabase.table("payments")
            .select("id, invoice_id, tenant_id, amount, discount_amount, created_at")
        ...
        payment_ids = [p["id"] for p in payments_in_range]
        already_allocated_ids = set()
        for i in range(0, len(payment_ids), 500):
            ...
            already_allocated_ids |= {r["payment_id"] for r in rows}

        missing = [p for p in payments_in_range if p["id"] not in already_allocated_ids]
        if not missing:
            return

        invoice_ids = list({p["invoice_id"] for p in missing})
        invoices = {
            inv["id"]: inv
            for inv in supabase.table("invoices").select("id, lease_id, company_id").in_("id", invoice_ids).execute().data
        }
...
def income_by_head(
    ...
    Self-healing: before reading, this also allocates any invoice-tied
    payment IN THIS DATE RANGE that doesn't have a payment_allocations row
    yet (e.g. it was recorded before this feature existed, or a rare
    best-effort allocation failed at the time). This is why the report
    should never come up empty just because nobody has run the manual
    /income-by-head/backfill endpoint -- that endpoint still exists for
    doing the same thing across a company's ENTIRE history in one go, but
    day-to-day viewing of this report no longer depends on it.
...
def income_by_head_drilldown(
    ...
    try:
        ...
        logging.getLogger("app.reports").exception("income_by_head_drilldown failed")
        raise HTTPException(status_code=500, detail=f"Couldn't load the receipts: {e}")
...
def backfill_income_by_head(
    ...
    Errors are caught PER PAYMENT, not for the whole batch -- one payment
    with a data problem (e.g. its invoice has zero line items) is recorded
    in `errors` below and skipped, rather than aborting every other payment
    that would otherwise have backfilled fine. The whole endpoint body is
    also wrapped so any unexpected failure comes back as a normal
    HTTPException (with a real message, and CORS headers, same as every
    other error response in this app) instead of an unhandled 500 -- the
    same class of bug already found and fixed once in record_receipt.
    """
    try:
        all_payments = (
            supabase.table("payments")
        ...
        if not all_payments:
            return {
                ...
                "payments_already_allocated": 0,
                "payments_skipped_no_line_items": 0,
                "errors": [],
            }
        ...
        processed, skipped_no_line_items, errors = 0, 0, []
        for p in all_payments:
            ...
            try:
                ...
                logging.getLogger("app.reports").exception("Backfill failed for payment_id=%s", p["id"])
                errors.append({"payment_id": p["id"], "invoice_id": p.get("invoice_id"), "error": str(e)})
        ...
        logging.getLogger("app.reports").exception("Backfill endpoint failed")
