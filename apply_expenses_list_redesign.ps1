# ============================================================================
# apply_expenses_list_redesign.ps1 -- same redesign as the Leases list page,
# applied to the Expenses list page:
#
#   1. No wrapping text anywhere -- long vendor names, building names, etc.
#      truncate with "..." or the table scrolls sideways instead of
#      breaking the layout.
#   2. A "Columns" dropdown (top right of the table) lets you show/hide
#      GL account, Paid from, Recurs, and Building / Split. Date, Category,
#      Vendor, and Amount always stay visible.
#   3. Click anywhere on a row to expand it and see the full detail: GL
#      account, Paid from, Building/Split, and the description (if any) --
#      useful even when you've hidden those columns above.
#   4. The old "->" dropdown menu (Edit / Manage split / View ledger) is
#      replaced with small icon buttons with hover tooltips -- a pencil
#      for Edit, a split icon for Manage split (only shown when the
#      expense isn't tied to one building), and a document icon for View
#      ledger. Matches the icon style already used on Owners and Leases.
#
# WHAT THIS DOES NOT CHANGE:
#   - Only one file changes: frontend/app/(dashboard)/expenses/page.tsx
#   - No backend files touched, no database changes, no API changes.
#   - Every action (Edit, Manage split, View ledger, Log expense, Generate
#     recurring) still does exactly what it did before -- just laid out
#     more cleanly.
#
# NO DATABASE CHANGES ARE NEEDED FOR THIS ONE.
#
# HOW TO USE:
#   1. Put this file AND "expenses_list_redesign.patch" into the main
#      folder of your project (the one with "backend" and "frontend"
#      inside it).
#   2. Open PowerShell in that folder.
#   3. Run:
#      powershell -ExecutionPolicy Bypass -File apply_expenses_list_redesign.ps1
# ============================================================================
...
if (-not (Test-Path ".\expenses_list_redesign.patch")) {
    Fail "Can't find 'expenses_list_redesign.patch' in this folder. Make sure you saved it next to this script before running it."
}
...
git apply --check expenses_list_redesign.patch
if ($LASTEXITCODE -ne 0) {
...
git apply expenses_list_redesign.patch
if ($LASTEXITCODE -ne 0) {
...
Remove-Item ".\expenses_list_redesign.patch" -ErrorAction SilentlyContinue
Write-Host "OK." -ForegroundColor Green

Write-Host ""
Write-Host "Step 6: Committing and pushing to GitHub..." -ForegroundColor Cyan
git add -u
git commit -m "Redesign Expenses list page: column picker, icon actions, expandable row detail, no text wrapping"
if ($LASTEXITCODE -ne 0) {
...
Write-Host "Then open the Expenses page: try the Columns dropdown, click a row to expand it," -ForegroundColor Green
Write-Host "and check the icon buttons (pencil = Edit, split = Manage split, document = View ledger)." -ForegroundColor Green
Write-Host "============================================================" -ForegroundColor Green
