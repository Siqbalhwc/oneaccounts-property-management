...
export default function LeaseSettlementPage() {
  ...
  return (
    <div className="max-w-4xl space-y-6">
      ...
      <Card>
        <Field label="Move-out date" hint="Defaults to today — change it and the figures below recalculate automatically.">
          <Input type="date" value={moveOutDate} onChange={(e) => setMoveOutDate(e.target.value)} className="max-w-xs" />
        </Field>
      </Card>
      ...
      {preview && (
        <>
          ...
          <Card className="bg-ledger/5 border-ledger/20">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-ink/50 mt-0.5">Deposit paid − deductions − discount − amount owed</p>
              </div>
              <span className={`text-xl font-display font-semibold figures ${projectedNet >= 0 ? "text-ledger" : "text-stamp-red"}`}>
          ...
          <div className="flex justify-end gap-2 pb-8">
            <Button variant="ghost" onClick={() => router.push("/leases")}>
              Cancel
            </Button>
